...
<div class="p-6 max-w-6xl mx-auto bg-white min-h-screen">
    ...
    <form action="<?php echo e(route('admin.layanan.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-5 bg-white p-6 border border-purple-100 rounded-none shadow-md">
        <?php echo csrf_field(); ?>

        
        <div>
            <label class="block mb-1 font-medium text-gray-700">Nama Layanan</label>
            <input type="text" name="nama" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" value="<?php echo e(old('nama')); ?>" required>
        </div>

        
        <div>
            <label class="block mb-1 font-medium text-gray-700">Deskripsi</label>
            <textarea name="deskripsi" rows="3" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" required><?php echo e(old('deskripsi')); ?></textarea>
        </div>

        
        <div>
            <label class="block mb-1 font-medium text-gray-700">Harga</label>
            <input type="number" name="harga" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" value="<?php echo e(old('harga')); ?>" required>
        </div>

        
        <div>
            <label class="block mb-1 font-medium text-gray-700">Tanggal</label>
            <input type="date" name="tanggal" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" value="<?php echo e(old('tanggal')); ?>" required>
        </div>
        ...
        <div>
            ...
            <select name="promo_id" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300">
                ...
                <?php $__currentLoopData = $promos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $promo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($promo->id); ?>" <?php echo e(old('promo_id') == $promo->id ? 'selected' : ''); ?>>
                        <?php echo e($promo->nama_promo); ?> - <?php echo e($promo->diskon); ?>% <?php if($promo->hanya_member): ?> (Member) <?php endif; ?>
                    </option>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ASVS\Documents\PBL S5\fix\abee\resources\views/admin/layanan/create.blade.php ENDPATH**/ ?>