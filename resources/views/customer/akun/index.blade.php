...
<div class="p-6 min-h-screen bg-white">
    ...
    <div class="max-w-4xl mx-auto bg-white border border-purple-100 shadow-sm rounded-sm p-6">
        ...
        {{-- Logika status membership aktif --}}
        @php
            use Carbon\Carbon;
            $isActive = $membership && Carbon::now('Asia/Jakarta')->lessThan($membership->expired_at);
        @endphp
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <input type="text"
                    value="{{ $isActive ? 'Member' : 'Belum Member' }}"
                    disabled
                    class="w-full bg-gray-100 border border-gray-300 rounded-md px-3 py-2 text-{{ $isActive ? 'green-600' : 'red-500' }} font-semibold">
            </div>
            ...
            @if ($isActive)
                ...
                <div>
                    ...
                    <input type="text"
                        value="{{ $membership->expired_at ? $membership->expired_at->timezone('Asia/Jakarta')->format('d-m-Y H:i') . ' WIB' : '-' }}"
                        disabled
                        class="w-full bg-gray-100 border border-gray-300 rounded-md px-3 py-2">
                </div>
            @endif
